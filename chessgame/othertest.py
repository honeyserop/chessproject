...
def create_room(color):
    ...
    e=Entry(join, text="enter id")
    e.pack()
    kp = 0
    room =Button(join, text="create room", command= lambda: creategame(e.get(),kp,color))
    room.pack()
...
def join_room():
    ...
    e.pack()
    kp = 0
    room = Button(join, text="join room", command=lambda: joingame(e.get(), kp))
    room.pack()
